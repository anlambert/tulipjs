...
//====================================================
static void identity(MatrixGL &m) {
  m.fill(0.0f);
  for (int i = 0 ; i < 4 ; ++i) {
    m[i][i] = 1.0f;
  }
...
Camera::Camera(Coord center,Coord eyes, Coord up, double zoomFactor, double sceneRadius):
  ...
  _3d(true),
  _viewOrtho(true) {
  identity(_modelviewMatrix);
  identity(_projectionMatrix);
  identity(_transformMatrix);
}
//====================================================
Camera::Camera(bool is3d) :
  _mdvMatCoherent(false), _projectionMatCoherent(false), _3d(is3d), _viewOrtho(true) {}
//====================================================
Camera::~Camera() {}
//===================================================
BoundingBox Camera::getBoundingBox() {
  BoundingBox bb;
  bb.expand(screenTo3DWorld(Coord(_viewport[0],_viewport[1],0)));
  bb.expand(screenTo3DWorld(Coord(_viewport[0]+_viewport[2],_viewport[1]+_viewport[3],0)));
  return bb;
}
//====================================================
void Camera::initGl() {
...
//    // Find the new x position for the new rotated point
//    vNewUp[0]  = (cosTheta + (1 - cosTheta) * x * x)    * _up[0];
//    vNewUp[0] += ((1 - cosTheta) * x * y - z * sinTheta)   * _up[1];
//    vNewUp[0] += ((1 - cosTheta) * x * z + y * sinTheta)   * _up[2];

//    // Find the new y position for the new rotated point
//    vNewUp[1]  = ((1 - cosTheta) * x * y + z * sinTheta)   * _up[0];
//    vNewUp[1] += (cosTheta + (1 - cosTheta) * y * y)          * _up[1];
//    vNewUp[1] += ((1 - cosTheta) * y * z - x * sinTheta)   * _up[2];

//    // Find the new z position for the new rotated point
//    vNewUp[2]  = ((1 - cosTheta) * x * z - y * sinTheta)   * _up[0];
//    vNewUp[2] += ((1 - cosTheta) * y * z + x * sinTheta)   * _up[1];
//    vNewUp[2] += (cosTheta + (1 - cosTheta) * z * z)          * _up[2];

//    // Now we just add the newly rotated vector to our position to set
//    // our new rotated eyes of our camera.
//    vNewEyes = _center + vNewEyes;

//    if (_eyes != vNewEyes || _up != vNewUp) {
//      _eyes = vNewEyes;
//      _up   = vNewUp;
//      _mdvMatCoherent=false;
//      notifyModified();
...
//}
//====================================================
void Camera::setViewOrtho(bool viewOrtho) {
  if (viewOrtho != _viewOrtho) {
